...
def parse_netlist(text):
    ...
    ValueError
        ...
        - Signal names are invalid (must be alphanumeric with underscores).
    ...
    for line_no, raw in enumerate(lines, start=1):
        ...
        # Ensure '#' does not appear after any non-space character
        if "#" in line and not line.lstrip().startswith("#"):
            raise ValueError(
                f"Line {line_no}: '#' must only be used for full-line comments"
            )
        header = line.split()[0]
    ...
    # Function to validate signal names (alphanumeric + underscores)
...
def topological_sort(parsed_data):
    ...
    while queue:
        ...
        for dependent in graph.get(var, []):
            indegree[
                dependent
            ] -= 1  # Since var is known, we can reduce indegree of unknown dependent variable
            if (
                indegree[dependent] == 0
            ):  # Now we completely know the previously unknown dependent variable
