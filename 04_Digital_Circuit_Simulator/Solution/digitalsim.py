...
def parse_netlist(text):
    ...
    def is_valid_signal_name(sig):
        if not sig:
            return False
        if not (sig[0].isalpha() or sig[0] == "_"):
            return False
...
def topological_sort(parsed_data):
    """
    Performs a simplified topological sort of the circuit gates based on signal dependencies.
    ...
    """
    declared_variables = set(parsed_data["INPUTS"].copy())
    new_order = []
    leftover_events = parsed_data["GATES"].copy()
    # As long as we have some leftover gates, we iterate over them
    while leftover_events:
        progress_made = False
        i = 0
        while i < len(leftover_events):
            # Get the inputs of the gate
            # elements of leftover_events have the structure [Output,gate,[inputs]]
            out, gate, inputs = leftover_events[i]
            # Below line checks if the inputs to current gate are already known.
            # If so, we add the gate's output to list of known variables and remove it from leftover_events
            if all(v in declared_variables for v in inputs):
                declared_variables.add(out)
                new_order.append(leftover_events.pop(i))
                progress_made = True
            else:
                # We increment i when we have NOT removed a element from leftover_events
                i += 1
        # Detect circular dependencies or undefined signals
        if not progress_made:
            raise ValueError(f"Circular dependency or undefined signal(s) detected:",leftover_events)
...
def to_wavedrom_json(data, waves):
    ...
    for timestep in waves:
        for v in ordered_signals:
            val = timestep.get(v)  # Get corresponding value of that variable in this timestep
